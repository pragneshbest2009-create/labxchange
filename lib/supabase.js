...
export function getSupabaseUnreachableMessage() {
  const host = getSupabaseHost();
  const base =
    'Supabase is unreachable — showing sample listings. Your database project may be deleted, paused, or misconfigured.';
  if (host) {
    return `${base} Configured host: ${host}. Create a new project at supabase.com/dashboard, run supabase/schema.sql, then update NEXT_PUBLIC_SUPABASE_URL and keys in Vercel → Settings → Environment Variables and redeploy.`;
  }
  return `${base} Add NEXT_PUBLIC_SUPABASE_URL and NEXT_PUBLIC_SUPABASE_ANON_KEY in Vercel → Settings → Environment Variables.`;
}
