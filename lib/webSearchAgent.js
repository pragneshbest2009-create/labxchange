...
import { classifyListing, parsePrice, inferCondition } from './classifyListing.js';
...
function buildQueries(term, category) {
  ...
  if (q) {
    return [
      `${q} used laboratory equipment for sale`,
      `${q} lab equipment labx equipnet machinio biosurplus`,
      `${q} refurbished lab instrument price`,
    ];
  }
